...
/**
 * Read one config var (non-empty). Prints NOTHING — this hook's contract
 * is empty stderr on every path. envBool()/envInt() route through this
 * helper, so INJECT, INJECT_LIMIT, INJECT_TTL_MS, PROJECT, URL and SECRET
...
async function main() {
  ...
  let url;
  try {
    url = new URL("memory/smart-search", base.endsWith("/") ? base : `${base}/`);
  } catch {
