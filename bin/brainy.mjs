#!/usr/bin/env node
/**
 * brainy — Brainy ops control plane CLI (SPEC-003-brainy-ops).
 *
 * Subcommands: add | move | distill | context | export | search | start | stop | status | doctor (+ --help).
...
function neverKillHint() {
  return (
    ...
    `start ours elsewhere: BRAINY_PORT=3151 npm run dev; ` +
    `then point clients at BRAINY_URL=http://127.0.0.1:3151 (example)`
  );
}
...
const USAGE = `brainy — ops control plane (SPEC-003)
...
Usage:
  ...
  brainy search "query" [--project P] [--limit N] [--include-graph] [--slot N]
  brainy start  --slot N [--data-dir PATH]
  brainy stop   --slot N [--data-dir PATH]
...
Env (BRAINY_* canonical; AGENT_MEMORY_* alias warns once on stderr):
  BRAINY_PORT / BRAINY_URL / HELIX_URL / BRAINY_DATA_DIR / BRAINY_SECRET / BRAINY_HOST / BRAINY_TTL_DAYS

Exit codes:
  add/move/distill/context/export/search : 0 ok · 1 failed · 2 usage
...
const FLAGS_BY_COMMAND = {
  ...
  search: new Set(["--slot", "--project", "--limit", "--include-graph"]),
...
function parseArgs(argv) {
  ...
  const head = argv[0];
  if (head === undefined) usageError("missing subcommand (add|move|distill|context|export|search|start|stop|status|doctor)");
  ...
  while (index < argv.length) {
    ...
    if (!token.startsWith("--")) {
      if (token.length === 0) usageError("empty positional argument");
    ...
    else if (name === "--title") flags.title = value;
    ...
    else if (name === "--limit") flags.limit = value;
    else if (name === "--include-graph") flags.includeGraph = true;
    index += 1;
  }
  ...
  const need = (cond, detail) => {
    if (!cond) usageError(detail);
  };
  let limit;
  if (flags.limit !== undefined) {
    need(/^[0-9]+$/.test(flags.limit), `invalid --limit "${flags.limit}" (integer 1..100)`);
    limit = Number(flags.limit);
    need(Number.isInteger(limit) && limit >= 1 && limit <= 100, `invalid --limit "${flags.limit}" (integer 1..100)`);
  }
  const assertNoPositionals = () => need(positionals.length === 0, `unexpected argument for "${command}"`);
  const assertOnePositional = (what) => need(positionals.length === 1, `${what} is required for "${command}"`);
  let text;
  let noteId;
  let query;
  switch (command) {
    case "add":
      assertOnePositional("note text");
      text = positionals[0];
      need(text.length <= 200_000, "note text exceeds 200000 chars");
      if (flags.title !== undefined) need(flags.title.length >= 1 && flags.title.length <= 500, "invalid --title (1..500 chars)");
      break;
    case "move":
    ...
    case "distill":
      ...
      noteId = positionals[0];
      need(noteId.length <= 200, "note id exceeds 200 chars");
      if (flags.provider !== undefined) need(["openai", "gemini", "anthropic"].includes(flags.provider), "invalid --provider (openai|gemini|anthropic)");
      if (flags.summary !== undefined) need(flags.summary.length >= 1 && flags.summary.length <= 200_000, "invalid --summary (1..200000 chars)");
      break;
    case "context":
      assertNoPositionals();
      need(flags.project !== undefined && flags.project.length > 0, '"context" requires "--project P"');
      break;
    case "export":
      assertNoPositionals();
      need(flags.format === "markdown", 'invalid --format (only "markdown")');
      need(flags.out !== undefined && flags.out.length > 0, '"export" requires "--out PATH"');
      need(flags.project !== undefined && flags.project.length > 0, '"export" requires "--project P"');
      break;
    case "search":
      assertOnePositional("search query");
      query = positionals[0];
      need(query.length <= 10_000, "search query exceeds 10000 chars");
      break;
    default:
      assertNoPositionals();
      break;
...
function resolveDataDir(slot, flagPath) {
  const checkRefused = (resolved) => {
    if (SYSTEM_ROOTS.has(resolved)) return "system root";
    if (resolved === path.resolve(HOME)) return "home directory itself";
    const underHome = resolved.startsWith(`${path.resolve(HOME)}${path.sep}`);
    const underTmp = resolved.startsWith(`/tmp${path.sep}`);
    if (!underHome && !underTmp) return "outside $HOME and /tmp";
    return undefined;
  };
  const checked = (raw, explicit, origin) => {
    const resolved = path.resolve(raw);
    const reason = checkRefused(resolved);
    if (reason !== undefined) return { path: resolved, explicit, origin, reason };
    return { path: resolved, explicit, origin };
  };
  // Precedence (ARCH §3): --data-dir flag > BRAINY_DATA_DIR > AGENT_MEMORY_DATA_DIR alias > default.
  if (flagPath !== undefined) return checked(flagPath, true, "--data-dir");
  const primary = nonEmpty(process.env["BRAINY_DATA_DIR"]);
  if (primary !== undefined) return checked(primary, true, "BRAINY_DATA_DIR");
  const legacy = nonEmpty(process.env["AGENT_MEMORY_DATA_DIR"]);
  if (legacy !== undefined) {
    warnAlias("BRAINY_DATA_DIR", "AGENT_MEMORY_DATA_DIR");
    return checked(legacy, true, "AGENT_MEMORY_DATA_DIR");
  }
  return { path: path.join(HOME, ".local", "share", "brainy", String(slot)), explicit: false, origin: "default" };
}
...
/**
 * Exactly one deprecation warning per legacy alias var per process (ARCH §3).
 * Format: `WARN deprecated use BRAINY_*` — never a value, never PII.
 */
const warnedAliases = new Set();
function warnAlias(canonical, legacy) {
  if (warnedAliases.has(legacy)) return;
  warnedAliases.add(legacy);
  err(`WARN deprecated use ${canonical} — ${legacy} alias will be removed in next major`);
}

/** Canonical env read with 1-version `AGENT_MEMORY_*` fallback (ARCH §3). */
function canonicalEnv(canonical, legacy) {
  const primary = nonEmpty(process.env[canonical]);
  if (primary !== undefined) return primary;
  if (legacy === undefined) return undefined;
  const fallback = nonEmpty(process.env[legacy]);
  if (fallback !== undefined) warnAlias(canonical, legacy);
  return fallback;
}

/** Bearer secret: `BRAINY_SECRET` primary, `AGENT_MEMORY_SECRET` alias — value never printed. */
function bearerSecret() {
  return canonicalEnv("BRAINY_SECRET", "AGENT_MEMORY_SECRET");
}

/** REST base URL: `BRAINY_URL` > `BRAINY_PORT` > aliases > slot derivation. */
function restBaseUrl(slot, ports) {
  const url = canonicalEnv("BRAINY_URL", "AGENT_MEMORY_URL");
  if (url !== undefined) return url.replace(/\/+$/, "");
  const port = canonicalEnv("BRAINY_PORT", "AGENT_MEMORY_PORT");
...
async function cmdStart(flags) {
  ...
  const brainyTtl = canonicalEnv("BRAINY_TTL_DAYS", "AGENT_MEMORY_TTL_DAYS") ?? "365";
  const brainyHost = canonicalEnv("BRAINY_HOST", "AGENT_MEMORY_HOST") ?? "127.0.0.1";
  const serverEnv = {
    ...process.env,
    BRAINY_PORT: String(ports.rest),
    BRAINY_HOST: brainyHost,
    BRAINY_URL: `http://127.0.0.1:${ports.rest}`,
    HELIX_URL: `http://127.0.0.1:${ports.helix}`,
    BRAINY_DATA_DIR: data.path,
    BRAINY_TTL_DAYS: brainyTtl,
    AGENT_MEMORY_PORT: String(ports.rest),
    AGENT_MEMORY_HOST: brainyHost,
  ...
  delete serverEnv["HELIX_DATA_DIR"];
  ...
  // --- readiness gate: Helix /healthz + ours /memory/livez and /v1/livez, 30 s
  const healthzUrl = `http://127.0.0.1:${ports.helix}/healthz`;
  const livezLegacyUrl = `http://127.0.0.1:${ports.rest}/memory/livez`;
  const livezV1Url = `http://127.0.0.1:${ports.rest}/v1/livez`;
  const deadline = Date.now() + READINESS_MS;
  let helixStatus = -1;
  let livezLegacyStatus = -1;
  let livezV1Status = -1;
  while (Date.now() < deadline) {
    if (childExit !== null) break;
    [helixStatus, livezLegacyStatus, livezV1Status] = await Promise.all([
      httpStatus(healthzUrl, { timeoutMs: 2_000 }),
      httpStatus(livezLegacyUrl, { timeoutMs: 2_000 }),
      httpStatus(livezV1Url, { timeoutMs: 2_000 }),
    ]);
    if (helixStatus === 200 && (livezLegacyStatus === 200 || livezV1Status === 200)) break;
    await sleep(250);
  }
  const livezStatus = livezLegacyStatus === 200 ? livezLegacyStatus : livezV1Status;
...
async function cmdStatus(flags) {
  ...
  const [helixStatus, livezLegacyStatus, livezV1Status, healthStatus] = await Promise.all([
    ...
    httpStatus(`http://127.0.0.1:${ports.rest}/v1/livez`, { timeoutMs: 2_500 }),
  ...
  out(`bearer: ${bearerSecret() === undefined ? "unset" : "armed"}`);
...
async function cmdDoctor(flags) {
  ...
  if (!restOwned) {
    ...
    const [livezLegacyStatus, livezV1Status, healthStatus] = await Promise.all([
      httpStatus(`http://127.0.0.1:${ports.rest}/memory/livez`, { timeoutMs: 3_000 }),
      httpStatus(`http://127.0.0.1:${ports.rest}/v1/livez`, { timeoutMs: 3_000 }),
...
/* ------------------------------------------------------------------ */
/* Thin REST clients: add|move|distill|context|export|search (ARCH §1)  */
/* No local state, no signals, no secrets in output — HTTP only.        */
/* ------------------------------------------------------------------ */

async function apiCall(base, apiPath, { method = "GET", body = undefined, secret = undefined, timeoutMs = 15_000 } = {}) {
  const headers = {};
  ...
  try {
    ...
    return { status: -1, json: undefined, text: "" };
  }
}

/** Narrow one string field out of an unknown JSON payload (never a secret). */
function jsonField(json, name) {
...
function apiRefused(base) {
  err(`REFUSE — REST unreachable at ${oneLine(base, 120)} (start the slot first)`);
  return 1;
}

function apiError(status, json, text) {
  const code = jsonField(json, "error");
  err(`REST ${status}: ${oneLine(code ?? text, 300)}`);
  return 1;
}
...
async function cmdAdd(flags) {
  ...
  const secret = bearerSecret();
  const tags = flags.tags === undefined ? undefined : parseTags(flags.tags);
  if (flags.tags !== undefined && tags === undefined) {
    err('usage: --tags takes a comma-separated non-empty list (e.g. --tags adr,helixdb)');
    err(USAGE);
    process.exit(2);
  }
  const firstLine = flags.text.split("\n")[0].trim().slice(0, 80);
  const title = flags.title ?? (firstLine.length > 0 ? firstLine : "Untitled");
  const result = await apiCall(base, "/v1/notes", {
    method: "POST",
    secret,
    body: {
      title,
  ...
  if (result.status === -1) return apiRefused(base);
  if (result.status !== 201) return apiError(result.status, result.json, result.text);
  out(`id: ${oneLine(jsonField(result.json, "id") ?? "unknown", 200)}`);
  out(`project: ${oneLine(jsonField(result.json, "project") ?? flags.project ?? "default", 200)}`);
  return 0;
}
...
async function cmdDistill(flags) {
  const ports = derive(flags.slot);
  const base = restBaseUrl(flags.slot, ports);
  const secret = bearerSecret();
  const project = flags.project ?? "default";
  const result = await apiCall(base, `/v1/notes/${encodeURIComponent(flags.noteId)}/distill`, {
    method: "POST",
    secret,
    body: { ...(flags.summary === undefined ? {} : { summary: flags.summary }), project },
  });
  if (result.status === -1) return apiRefused(base);
  if (result.status !== 201) return apiError(result.status, result.json, result.text);
  const note = typeof result.json === "object" && result.json !== null && "note" in result.json ? result.json.note : undefined;
  out(`distilled: ${oneLine(flags.noteId, 120)} -> ${oneLine(jsonField(note, "id") ?? "unknown", 120)}`);
  return 0;
}
...
async function cmdContext(flags) {
  ...
  const result = await apiCall(
    base,
    `/v1/context/${encodeURIComponent(flags.project)}?limit=${limit}`,
    { secret },
  );
  if (result.status === -1) return apiRefused(base);
  if (result.status !== 200) return apiError(result.status, result.json, result.text);
  const notes = typeof result.json === "object" && result.json !== null && Array.isArray(result.json.notes) ? result.json.notes : [];
  out(`project: ${oneLine(flags.project, 200)}`);
  out(`notes: ${notes.length}`);
  for (const note of notes.slice(0, limit)) {
    if (typeof note !== "object" || note === null) continue;
    out(`- ${oneLine(jsonField(note, "id") ?? "?", 80)} ${oneLine(jsonField(note, "title") ?? "(untitled)", 120)}`);
  }
  return 0;
}

function slugifyTitle(title) {
  const slug = String(title).toLowerCase().replace(/[^a-z0-9-_]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60);
...
async function cmdExport(flags) {
  ...
  const base = restBaseUrl(flags.slot, ports);
  const secret = bearerSecret();
  const result = await apiCall(base, `/v1/context/${encodeURIComponent(flags.project)}?limit=100`, { secret });
  if (result.status === -1) return apiRefused(base);
  if (result.status !== 200) return apiError(result.status, result.json, result.text);
  const notes = typeof result.json === "object" && result.json !== null && Array.isArray(result.json.notes) ? result.json.notes : [];
  ...
  if (stat !== undefined && !stat.isDirectory()) {
    err(`REFUSE export — --out is not a directory: ${collapse(outDir)}`);
    return 1;
  }
  ...
  for (const note of notes) {
    if (typeof note !== "object" || note === null) continue;
    const id = jsonField(note, "id") ?? `note-${count}`;
    const file = path.join(outDir, `${slugifyTitle(jsonField(note, "title") ?? "untitled")}-${id.slice(0, 8)}.md`);
    if (!file.startsWith(`${outDir}${path.sep}`)) continue;
    const tags = Array.isArray(note.tags) ? note.tags.filter((t) => typeof t === "string").slice(0, 64) : [];
    const body = [`---`, `project: ${flags.project}`, `tags: [${tags.join(", ")}]`, `---`, ``, String(jsonField(note, "content") ?? ""), ``].join("\n");
  ...
  out(`exported: ${count} notes -> ${collapse(outDir)}`);
  return 0;
}
...
async function cmdSearch(flags) {
  ...
  const base = restBaseUrl(flags.slot, ports);
  const secret = bearerSecret();
  const result = await apiCall(base, "/v1/search", {
    method: "POST",
    secret,
    body: {
      query: flags.query,
      ...
      ...(flags.includeGraph ? { include_graph: true } : {}),
    },
  });
  if (result.status === -1) return apiRefused(base);
  if (result.status !== 200) return apiError(result.status, result.json, result.text);
  const results = typeof result.json === "object" && result.json !== null && Array.isArray(result.json.results) ? result.json.results : [];
  out(`results: ${results.length}`);
  for (const row of results) {
    if (typeof row !== "object" || row === null) continue;
    const note = typeof row.note === "object" && row.note !== null ? row.note : row;
    out(`- ${oneLine(jsonField(note, "id") ?? "?", 80)} score=${oneLine(jsonField(row, "score") ?? "?", 24)} ${oneLine(jsonField(note, "title") ?? "", 120)}`);
  }
  return 0;
}
...
async function main() {
  ...
  switch (flags.command) {
    case "add":
      return await cmdAdd(flags);
    case "move":
      return await cmdMove(flags);
    case "distill":
      return await cmdDistill(flags);
    case "context":
      return await cmdContext(flags);
    case "export":
    ...
    default:
      usageError("missing subcommand (add|move|distill|context|export|search|start|stop|status|doctor)");
      return 2;
  }
}
