...
**v1.8 amendment (2026-09-25, SPEC-005 Brainy v1 legal — Note.content PII declaration + ARCO SLA home):** §3 grows the Brainy v1 `Note.content` / compat `Memory.statement` PII declaration in Ley 172-13 shape, mirroring the v1.4 Concept-retention block above (purpose + legal basis + minimization + TTL + two-half deletion + honest boundary). Docs-only declaration: no route, MCP tool, or code change; closes DAT-001-style for Note. Approval basis (SPEC-005 REQ-BRAINY-LEG-01): "The declaration lives in `ARCHITECTURE.md` NFR Security row + this SPEC §4.1 (canonical) and is mirrored in `docs/CONTRACT.md` Brainy v1 amendment (docs-only, no route change, closes DAT-001-style for Note)". Canonical source is SPEC-005 §4.1, quoted verbatim below (CDR-01-C1 — no paraphrase drift on purpose/TTL/deletion numbers):
1. **Purpose:** `segundo cerebro personal CODE/PARA` — captura/organiza/destila/expresa conocimiento persistente para agentes (Projects/Areas/Resources/Archives), elimina amnesia de sesión. **Legal basis:** consent + legitimate interest (single-tenant local; caller-supplied `tags`/`concepts` verbatim = caller's responsibility — same rule as `content`).
2. **Minimization:** text-only MVP (no images/audio, BRIEF Out-of-Scope); `title 1..500` `content 1..200k` `tags string[64] 1..200` strict zod; `concepts/tags ≤8` (derived `extractConcepts` tf-rank ≤8 or explicit verbatim); no collection beyond `Note{noteId,title,content,project,embedding[1536],created_at,updated_at,status,paraType}`; compat `Memory.statement 1..200k` alias `content`, `concepts ≤8` or caller verbatim, `DEDUP first-wins` per §3. Derived Concept terms (`extractConcepts` stopword/length-filtered ≤8) are filtered tokens — PII scan `0/0` at gate P1R-P32 W-3; caller-supplied labels stored verbatim are the CALLER's responsibility.
3. **TTL:** `BRAINY_TTL_DAYS` default `365` (alias `AGENT_MEMORY_TTL_DAYS` 1 version + warning; absent/invalid/≤0 → OFF declared). Enforcement is `filterExpired` (`ageDays > ttlDays` strictly, unparseable `createdAt` → kept) with `signals: ["ttl: hidden N expired rows"]` when N>0 — explicit degradation, never silent thinning.
4. **Deletion procedure (the two halves):** (a) Reachable erasure surfaces in v1 — Memory rows: `forgetMemory` / `POST /memory/forget` (compat, audit-silent — not SLA evidence) / `POST /memory/delete {memoryId,reason}` (governed path with receipt, SLA evidence) / `scripts/purge.ts --days N --project P|--all` (batches 500 `listExpired` + `forgetMemory` per id, allowlist output) / `DELETE /memory/todos/:id` (Todo rows). Note rows: controller-executed erasure via project-guarded store-level `forgetNote` within the ARCO SLA (item 5 below) — no REST/MCP/CLI self-service surface in v1 (zero refs in `src/server.ts`, `src/mcp.ts`, `scripts/purge.ts`, `bin/brainy.mjs`); REST/MCP exposure plus a `purge.ts` Note branch are deferred to the next erasure-surface change or v2 (R4 adjudication item 3); (b) operator-run manual orphan `Concept` cleanup, 3-step audit→gate→drop→re-audit, no scheduler in Brainy v1 (CONTRACT v1.4 procedure, re-run on demand). **Honest boundary:** a `Concept.name` still referenced by other rows cannot be dropped without breaking those rows (accepted, derived tokens non-PII W-3); shared vocabulary TTL none intentional (global uniqueness `Concept.name`, cardinality-bounded).
5. **ARCO SLA home (CDR-01-C2):** data-subject requests acknowledge within ≤5 business days and resolve within ≤15 business days; owner `subero`; any delay beyond triggers escalation to the orchestrator.
6. **Erasure-evidence boundary (CDR-03-C1):** ARCO erasure requests are served via the governed path — `POST /memory/delete {memoryId,reason}` (governance line `memoryId`+`reason`+`at`, receipt `{memoryId,deletedAt}` omits `reason` by design) or the `scripts/purge.ts` bulk path; `POST /memory/forget` is a compat path and is not sufficient for SLA-bound erasure evidence. Example shapes placeholder-only: `memoryId=<note-id>`, bearer `$BRAINY_SECRET` — never values, never raw content.
