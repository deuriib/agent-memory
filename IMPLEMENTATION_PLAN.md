...
- [x] Ops/persistence: save → `helix restart dev` → same memory still searchable (P0.4 acceptance) — original canary `228cdf69` BM25 0.863 post-restart, storage stayed `disk`; **artifact-backed rerun** 2026-09-22: token `p04canary1790108269` found on the first post-restart attempt, full log `docs/specs/40_workspace/quality-gate/P0/evidence/p0-4-restart-canary.log` (closes QA-04 single-source)
